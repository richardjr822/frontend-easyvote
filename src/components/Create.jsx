...
const Create = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure POST is used for the request
      const response = await axios.post("http:///127.0.0.1:8000//api/candidates", formDataToSend, {
        headers: {
          "Content-Type": "multipart/form-data", // For file uploads
        },
      });
    
      // Handle response and success logic here...
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 md:col-span-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Two columns for Position and partylist */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className={`relative ${errors.partylist ? 'mb-6' : ''}`}>
                    <select
                      id="partylist"
                      name="partylist"
                      value={formData.partylist}
                      onChange={handleInputChange}
                      className={`block w-full border ${errors.partylist ? 'border-red-500' : 'border-gray-300'} rounded-lg shadow-sm px-4 py-3 bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors appearance-none`}
                    >
                    ...
                    {errors.partylist && (
                      <div className="absolute -bottom-6 left-0 text-red-500 text-xs flex items-center">
                        <FaExclamationCircle className="mr-1" /> {errors.partylist}
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div className="bg-gray-50 rounded p-2">
                        ...
                        <p className="font-medium text-gray-800">{candidate.partylist}</p>
                      </div>
                    </div>
                  </div>
                ))}
